  ...
  describe("Memory Game", () => {
    ...
    const flipAllMatchingPairs = () => {
      mockCardsArray.forEach((card) => {
        ...
        const matchingCard = mockCardsArray.find(
          (matchCard) =>
            matchCard.dataset.symbol === card.dataset.symbol &&
            matchCard !== card
        );
        if (matchingCard) {
          matchingCard.click();
    ...
    beforeEach(() => {
      ...
      matchCard2 = mockCardsArray.find(
        (card) =>
          card.dataset.symbol === matchCard1.dataset.symbol &&
          card !== matchCard1
      );
    ...
    describe("Card Interaction", () => {
      ...
      it("should not allow flipping more than two cards at the same time", () => {
        ...
        expect(memoryGame.flipCard).toHaveBeenCalledWith(card2);
        expect(memoryGame.flipCard).not.toHaveBeenCalledWith(card3);
      });
